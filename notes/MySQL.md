...
- 数据组装：可以将原来的连接分解成多个单表查询，然后在用户程序中进行连接。
- 全局表： 对系统中所有模块都可能依赖的一些表，为避免跨库查询，将这类表在每个数据库中都保存一份
- 字段冗余： 典型的反范式设计，利用空间换时间。比如订单存userId时也将userName也冗余保存一份，这样查询订单时就不需要再去查userName所在的表
- ER分片：将存在关联关系的表记录存放在一个分片上
